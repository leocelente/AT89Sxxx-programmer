...
class Programmer:
    ...
    def setSerialPort(self, portname: str) -> bool:
        ...
        linux = portname.startswith(
            "/dev/tty") or portname.startswith("/dev/pts")
    ...
    def setHexFile(self, file: TextIOWrapper) -> None:
        ...
        self.file_contents = "".join(file.readlines()).replace(
            ':', '').replace('\n', '')
    ...
    def sendProgram(self, onUpdate: Callable[[float], None]) -> None:
        ...
        def delay_ms(ms): return time.sleep(ms/1000)
